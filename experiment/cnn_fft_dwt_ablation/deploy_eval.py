...
Usage: uv run python experiment/cnn_fft_dwt_ablation/deploy_eval.py
"""
import json
import sys
import time
...
from inference.config import configure  # noqa: E402
from inference.predictor import ScreenDetectorPredictor  # noqa: E402
...
def main() -> None:
    print(f"=== ONNX deploy eval ===")
    print(f"model: {ONNX_PATH}")
    print(f"size: {ONNX_PATH.stat().st_size / 1e6:.1f} MB")
    if not ONNX_PATH.exists():
        raise FileNotFoundError(f"ONNX missing at {ONNX_PATH}")
    ...
    pred = ScreenDetectorPredictor(model_path=ONNX_PATH)
    ...
    # Treat OOD (unknown) conservatively: any unknown for screen_photo GT counts
    # as miss; for non-screen GT counts as a fallback to the most likely class.
    # Report metrics both as raw (incl. unknowns) and corrected (unknown->screen_photo)
    ...
    # Corrected view: OOD becomes screen_photo (matches production fall-through to safe class)
    ...
    print("-" * 70)
    print(f"{'raw (unknown counted)':<22} {raw_acc:>7.4f} {sp_f1:>7.4f} {sp_p:>6.3f} {sp_r:>6.3f} {raw_macro:>9.4f} {raw_metric:>7.4f}")
    print(f"{'unknown->screen_photo':<22} {cor_acc:>7.4f} {sp_f1_c:>7.4f} {sp_p_c:>6.3f} {sp_r_c:>6.3f} {cor_macro:>9.4f} {cor_metric:>7.4f}")
    print("=" * 70)
    ...
    out = {
        ...
        "n_test": len(test),
        "raw": {
            "accuracy": raw_acc, "sp_f1": sp_f1, "sp_precision": sp_p, "sp_recall": sp_r,
            "macro_f1": raw_macro, "metric": raw_metric,
        },
        "ood_as_screen_photo": {
            "accuracy": cor_acc, "sp_f1": sp_f1_c, "sp_precision": sp_p_c, "sp_recall": sp_r_c,
            "macro_f1": cor_macro, "metric": cor_metric,
        },
        "latency_ms": {
    ...
    out_path = ROOT / "experiment" / "cnn_fft_dwt_ablation" / "deploy_eval.json"
